    ...
    <div style={{padding:16}}>
      ...
      {items.length === 0 ? <p>No bookings yet</p> : (
        <div style={{display:'grid',gap:8}}>
          {items.map(b=>(
            <div key={b._id} style={{border:'1px solid #eee',padding:8,borderRadius:8,display:'flex',justifyContent:'space-between'}}>
              <div>
                <div><b>{b.therapist?.name || b.therapist}</b></div>
                <div>{b.slot && new Date(b.slot.start).toLocaleString()} — {b.status}</div>
              </div>
              <button onClick={()=>cancel(b._id)} style={{color:'crimson'}}>Cancel</button>
            </div>
          ))}
        </div>
      )}
