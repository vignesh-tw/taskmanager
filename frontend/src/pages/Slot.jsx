...
export default function Slots() {
  ...
  const loadSlots = async () => sel ? setSlots(await listSlots({ therapistId: sel, onlyOpen: 'true' })) : setSlots([]);

  useEffect(()=>{ loadTherapists(); }, []);
  useEffect(()=>{ loadSlots(); }, [sel]);
  ...
  const add = async (e) => {
    ...
    try {
      await createSlot({ therapistId: sel, start: toISO(form.start), end: toISO(form.end) });
      setForm({ start:'', end:'' }); setMsg('Slot created'); loadSlots();
    } catch(e){ setMsg(e.message); }
  };

  const book = async (s) => {
    try {
      await createBooking({ therapistId: s.therapist, slotId: s._id, notes: '' });
      setMsg('Booked'); loadSlots();
    } catch(e){ setMsg(e.message); }
  };
  ...
  return (
    <div style={{padding:16}}>
      ...
      <select value={sel} onChange={e=>setSel(e.target.value)} style={{marginBottom:12}}>
        ...
        {therapists.map(t => <option key={t._id} value={t._id}>{t.name} (${t.rate})</option>)}
      </select>

      {/* Create slot */}
      <form onSubmit={add} style={{display:'flex',gap:8,alignItems:'center',marginBottom:12}}>
      ...
      {/* List + actions */}
      {!sel ? <p>Select therapist to see open slots</p> : (
        <table style={{width:'100%',borderCollapse:'collapse'}}>
          <thead><tr><th align="left">Start</th><th>End</th><th>Actions</th></tr></thead>
          <tbody>
            {slots.map(s=>(
              <tr key={s._id} style={{borderTop:'1px solid #eee'}}>
                ...
                <td align="right" style={{whiteSpace:'nowrap'}}>
                  ...
                  <button onClick={()=>remove(s._id)} style={{color:'crimson'}}>Delete</button>
                </td>
              </tr>
            ))}
            {slots.length===0 && <tr><td colSpan="3">No open slots</td></tr>}
          </tbody>
        </table>
      )}
    </div>
