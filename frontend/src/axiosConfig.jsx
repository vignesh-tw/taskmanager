...
const inferredBase = (() => {
  ...
  if (typeof window !== 'undefined' && window.location && /:3000$/.test(window.location.origin)) {
...
if (typeof window !== 'undefined') {
  (async () => {
    try {
      ...
      console.warn('[axios][startup] Cannot reach API base:', inferredBase, e.message);
    }
  })();
}
...
axiosInstance.interceptors.request.use(
  (config) => {
    ...
    if (/\/auth\/register$/.test(config.url || '')) {
      ...
      const payload = typeof config.data === 'string' ? config.data : { ...(config.data || {}) };
      if (payload.password) payload.password = '***';
      console.log('[axios][register][request]', { baseURL: config.baseURL, url: config.url, method: config.method, data: payload });
    }
    return config;
  },
...
axiosInstance.interceptors.response.use(
  ...
  (error) => {
    ...
    if (error.response?.data && !error.response.data.normalizedMessage) {
      error.response.data.normalizedMessage = error.response.data.message || 'Request failed';
    }
