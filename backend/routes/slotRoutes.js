...
class UpdateSlotController {
  async execute(req, res) {
    try {
      ...
      const updatedSlot = await slotRepository.updateStatus(
        req.params.id,
        status,
      );
...
const createSlot = new AuthenticationDecorator(
  new RoleAuthorizationDecorator(new CreateSlotController(), ["therapist"]),
);
const getSlot = new AuthenticationDecorator(new GetSlotController());
const updateSlot = new AuthenticationDecorator(
  new RoleAuthorizationDecorator(new UpdateSlotController(), ["therapist"]),
);
