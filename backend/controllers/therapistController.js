...
const getTherapistById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching therapist", error: error.message });
  }
};
...
const updateAvailability = async (req, res) => {
  try {
    ...
    const hasValidDays = Object.keys(availability).every((day) =>
    ...
    const hasValidTimeSlots = Object.values(availability).every(
      (slots) =>
        Array.isArray(slots) &&
        slots.every(
          (slot) =>
            slot &&
            isValidTimeFormat(slot.start) &&
            isValidTimeFormat(slot.end),
        ),
    );
