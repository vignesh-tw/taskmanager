...
bookingSchema.methods.cancel = async function(reason) {
  ...
  if (process.env.NODE_ENV !== 'test') {
    ...
    if (hoursUntilAppointment < 24) {
      throw new Error('Bookings cannot be cancelled less than 24 hours before appointment');
    }
  }
...
bookingSchema.statics.findByDateRange = function(userId, userType, startDate, endDate) {
  return this.find({
    [userType]: userId,
    createdAt: {
      $gte: startDate,
