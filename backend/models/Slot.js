  ...
  status: {
    ...
    validate: {
      validator: function(value) {
        ...
        return ['available', 'booked', 'cancelled', 'completed'].includes(value);
      },
...
slotSchema.pre('save', function(next) {
  ...
  if (startHour < 9 || endHour > 17) {
    const error = new Error('Slots must be within business hours (9 AM to 5 PM)');
    error.status = 400;
    return next(error);
  }
...
slotSchema.statics.findAvailableSlots = function(therapistId, startDate, endDate) {
  return this.find({
