...
describe("Smoke Tests", () => {
  ...
  describe("API Endpoints", () => {
    it("auth endpoint should be accessible", async () => {
      await request(app).post("/api/auth/login").expect(400); // Expect 400 as no credentials provided
    });

    it("tasks endpoint should require auth", async () => {
      await request(app).get("/api/tasks").expect(401); // Expect 401 as no token provided
    });

    it("therapists endpoint should be accessible", async () => {
      await request(app).get("/api/therapists").expect(200);
    });
  });
});
